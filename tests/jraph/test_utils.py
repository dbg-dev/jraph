...
class GraphTest(parameterized.TestCase):
    ...
    def test_batch_unbatch_with_random_graphs(
        self, include_globals, include_nodes, include_edges
    ):
    ...
    def test_pad_with_graphs(self):
        ...
        expected_padded_graph = graph.GraphsTuple(
            ...
            receivers=jnp.concatenate(
                [graphs_tuple.receivers, jnp.array([7, 7, 7, 7])]
            ),
        )
        jax.tree_util.tree_map(
    ...
    def test_pad_unpad_with_random_graphs(
        self, include_globals, include_nodes, include_edges
    ):
    ...
    def test_segment_max_or_constant_2d(self, indices_are_sorted, unique_indices):
        if unique_indices:
            ...
            if indices_are_sorted:
                ...
                segment_ids = jnp.array([1, 0, 2, 4, 3, -5])
                expected_out = jnp.array([[1, 5], [0, 6], [2, 4], [4, 2], [3, 3]])
                num_segments = 5
        else:
    ...
    def test_segment_softmax(self, nan_data):
        ...
        with self.subTest("nojit"):
            ...
            np.testing.assert_allclose(result, expected_out)
        with self.subTest("jit"):
            result = jax.jit(utils.segment_softmax, static_argnums=2)(
    ...
    @parameterized.named_parameters(
        ("valid_1_no_feat", 1, 1, False, False),
    ...
    def test_fully_connected_graph(self, n_node, n_graph, nodes, globals_):
        ...
        with self.subTest("senders_receiver_indices"):
            if n_node > 0:
                ...
                expected_receivers = np.stack([node_indices] * n_node, axis=-1).reshape(
                    [-1]
                )
            else:
    ...
    def test_fully_connected_graph_order_edges(self, add_self_edges):
        ...
        graph_batch = utils.get_fully_connected_graph(
            n_node_per_graph=3, n_graph=1, add_self_edges=add_self_edges
        )

        if add_self_edges:
            np.testing.assert_array_equal(graph_batch.senders, [0, 1, 2] * 3)
...
def _make_dynamic_batch_graph(
    ...
    g_ = _make_nest(np.random.normal(size=_DB_GLOBAL_SHAPE)) if add_globals else {}
    return graph.GraphsTuple(
        nodes=_make_nest(np.random.normal(size=(total_num_nodes,) + _DB_NODE_SHAPE)),
        edges=_make_nest(np.random.normal(size=(total_num_edges,) + _DB_EDGE_SHAPE)),
...
class DynamicBatchTest(parameterized.TestCase):
    ...
    @parameterized.named_parameters(
        ...
        (
            "graph_with_globals_n_graph_hit",
            True,
            {
                "n_node": sum(_DB_NUM_NODES) + 100,
                "n_edge": sum(_DB_NUM_EDGES) + 100,
    ...
    def test_dynamically_batch(self, use_globals, batch_kwargs):
        ...
        batched_dataset = utils.dynamically_batch(graph_iterator(), **batch_kwargs)
        batched_graphs = []
        while True:
            try:
...
class ZeroOutTest(parameterized.TestCase):
    def _assert_values_for_graph(self, padded_graph, wrapper):
        ...
        padded_graph = padded_graph._replace(
            nodes=tree.tree_map(lambda x: x - 1.0, padded_graph.nodes),
            edges=tree.tree_map(lambda x: x - 1.0, padded_graph.edges),
            globals=tree.tree_map(lambda x: x - 1.0, padded_graph.globals),
        )
        ...
        tree.tree_map(
            np.testing.assert_array_equal, valid_graph.nodes, true_valid_graph.nodes
        )
...
class AdjacencyMatrixTest(parameterized.TestCase):
    def test_sparse_matrix_to_graphs_tuple(self):
        ...
        for sparse_matrix, expected_graph in zip(sparse_adj_matrices, expected_graphs):
            ...
            jax.tree_util.tree_map(
                np.testing.assert_allclose, from_sparse_graph, expected_graph
            )
