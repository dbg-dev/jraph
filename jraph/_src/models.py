...
def InteractionNetwork(
    ...
    if include_sent_messages_in_node_update:
        wrapped_update_node_fn = lambda n, s, r, g: update_node_fn(
            n, s, r
...
def GraphMapFeatures(
    embed_edge_fn: EmbedEdgeFn | None = None,
...
type GATAttentionQueryFn = Callable[[NodeFeatures], NodeFeatures]
type GATAttentionLogitFn = Callable[
    [SenderFeatures, ReceiverFeatures, EdgeFeatures], EdgeFeatures
]
type GATNodeUpdateFn = Callable[[NodeFeatures], NodeFeatures]


def GAT(
    attention_query_fn: GATAttentionQueryFn,
    attention_logit_fn: GATAttentionLogitFn,
    node_update_fn: GATNodeUpdateFn | None = None,
) -> Callable[..., GraphsTuple]:
