...
def segment_min_or_constant(
    ...
    return _replace_empty_segments_with_constant(
        mins_, segment_ids, num_segments, constant
    )
...
def partition_softmax(
    ...
    segment_ids = jnp.repeat(
        jnp.arange(n_partitions), partitions, axis=0, total_repeat_length=sum_partitions
    )
...
def unpad_with_graphs(padded_graph: GraphsTuple) -> GraphsTuple:
    ...
    unpadded_graph = GraphsTuple(
        ...
        globals=tree.tree_map(lambda x: x[:-n_padding_graph], padded_graph.globals),
...
def get_fully_connected_graph(
    ...
    if not add_self_edges:
        tmp_senders = jax.vmap(jnp.roll)(tmp_senders, -jnp.arange(len(tmp_senders)))[
            :, 1:
        ]
