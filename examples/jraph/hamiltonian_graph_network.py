...
def hookes_hamiltonian_from_graph_fn(graph: GraphsTuple) -> GraphsTuple:
...
def get_random_uniform_norm2d_vectors(
    min_norm: float, max_norm: float, num_particles: int
) -> np.ndarray:
...
def set_system_state(
    static_graph: GraphsTuple, position: np.ndarray, momentum: np.ndarray
) -> GraphsTuple:
...
def single_integration_step(
    ...
    hamiltonian_fn = get_hamiltonian_from_state_fn(
        static_graph=static_graph, hamiltonian_from_graph_fn=hamiltonian_from_graph_fn
    )
    state_derivatives_fn = get_state_derivatives_from_hamiltonian_fn(hamiltonian_fn)
...
def main() -> None:
    ...
    axes[0].plot(positions_sequence_array[..., 0], positions_sequence_array[..., -1])
